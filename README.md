...
## Running the demo (X minutes)
There are 2 datasets used for the demo. For analyses involving neural data, a subset of the majajhong dataset (50 images) is used. For analyses relating to image classification, a subset of the Places train set (500 images) and validation set (500 images) is used. Further, the smallest version of each untrained model is considered for all parts of the demo. Each demo has a separate notebook in the folder ```demo_notebooks```, where the output can be visualized. Alternatively, the scripts for generating demo results can be run as shown below. Navigate to the demo folder, then:

To generate figure 2, 4 and S7, run (total time: X seconds): 
```
python main/engineered_models.py
python supplementary/sota_models.py --model alexnet_trained
```

To generate figure 3, run (total time: X seconds):
```
python main/pca_analysis.py
```

To generate figure 5, run (total time: 80 seconds):
```
python demo/ablation_studies.py
```

To generate figure 6, run: (total time: X minutes):
```
python demo/classification.py 
```

Similarly, demos for the supplemetary figures can be generated by running the scripts in the supplementary folder (refer to README in ```demo/supplementary```). 
