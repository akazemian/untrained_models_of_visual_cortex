...
class Model(nn.Module):
    ...
    def forward(self, x:nn.Module):
        ...
        for i in range(self.batches_4):
            conv_4.append(self.c4(x)) 
        x = torch.cat(conv_4,dim=1)
        if self.print_shape:
            print('conv4', x.shape)
...
class EngineeredModel4L:
    ...
    def __init__(self, curv_params = {'n_ories':8,'n_curves':3,'gau_sizes':(5,),'spatial_fre':[1.2]},
                 filters_2=2000,filters_3=2000,filters_4=10000,batches_4 = 1):
    ...
    def Build(self):
        ...
        c1 = StandardConvolution(filter_size=9,filter_type='curvature',curv_params=self.curv_params)     
        mp1 = nn.MaxPool2d(kernel_size=3)
        
        c2 = nn.Conv2d(24, self.filters_2, kernel_size=(5, 5))
        mp2 = nn.MaxPool2d(kernel_size=2)
        
        c3 = nn.Conv2d(self.filters_2, self.filters_3, kernel_size=(3, 3))
        mp3 = nn.MaxPool2d(kernel_size=2)
        
        c4 = nn.Conv2d(self.filters_3, self.filters_4, kernel_size=(3, 3))
        mp4 = nn.MaxPool2d(kernel_size=3)
        
        last = Output()

        return Model(c1,mp1,c2,mp2,c3,mp3,c4,mp4,self.batches_4,last)  
