...
class Model(nn.Module):
    ...
    def __init__(self,
        ...
        self.M = M
        self.N = N
        self.filter_set = FiltersSet(self.M, self.N, self.J, self.L).generate_morlet(precision='single')
    ...
    def forward(self, x:nn.Module):
        ...
        N = x.shape[0]
        x = x.squeeze()
        ST_calculator = ST_2D(self.filter_set, self.J, self.L, device='gpu', weight=None)
        S, S0, S1, S2 = ST_calculator.forward(x, self.J, self.L)
        x = torch.cat([S1.reshape(N,-1),S2.reshape(N,-1)],dim=1)
...
class ScatTransform():
    ...
    def __init__(self, J, L, M, N, num_projections=None):
