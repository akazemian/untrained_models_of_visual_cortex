...
for model_name in MODELS:
    ...
    for features in FEATURES:
        ...
        pca_iden =activations_identifier = load_full_identifier(model_name=model_name, features=features, 
                                                            layers=5, dataset=DATASET, components = [i])
        print(iden)
        
        if DATASET == 'naturalscenes':
            data = filter_activations(data, NSD_UNSHARED_SAMPLE)
            _PCA(n_components = COMPONENTS[i])._fit(pca_iden, data)
