...
def compute_model_pcs(model_name:str, features:int, layers:int, batch_size:int,
    ...
    #extract activations if not saved already
    ...
    if dataset == 'naturalscenes':
        ...
        NSD_UNSHARED_SAMPLE = [image_id.strip('.png') for image_id in IDS_PATH]#[:10000]
    ...
    # compute pcs
    _pca = _PCA(n_components = components)
    # _pca.set_components(data)
    ...
    if incremental:
        pca_iden += '_incremental'
    
    print(pca_iden)
    _pca._fit(pca_iden, data, incremental)
    del _pca
