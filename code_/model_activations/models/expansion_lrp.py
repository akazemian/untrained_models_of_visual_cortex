...
class Model(nn.Module):
    ...
    def __init__(self, conv1: nn.Module, pool1: nn.Module, conv2: nn.Module, pool2: nn.Module, 
                 conv3: nn.Module, pool3: nn.Module, conv4: nn.Module, pool4: nn.Module, lin5: nn.Module,
                 nl: nn.Module, last: nn.Module, device: str) -> None:
...
class ExpansionLRP:
    ...
    def __init__(self, filters_1: Optional[int] = None, filters_2: int = 256, filters_3: int = 256, 
                 filters_4: int = 256, filters_5: int = 30000, init_type: str = 'kaiming_uniform', 
    ...
    def build(self):
        ...
        lin5 = nn.Linear(self.filters_4 * 11**2, self.filters_5)

        nl = NonLinearity(self.non_linearity)
        last = Output()

        return Model(
            conv1, pool1, conv2, pool2, conv3, pool3, conv4, pool4, lin5, nl, last, self.device
        )
